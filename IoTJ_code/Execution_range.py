...
total_cen_signal_overheads=[0 for i in range(0,len(user_ranges))]
total_decen_signal_overheads=[0 for i in range(0,len(user_ranges))]
...
def execute(iteration_time):
    ...
    result_file = open('Experiment/range.docx', 'w')
    ...
    for iteration in range(0,iteration_time):
        ...
        for user_id in range(0, user_num):
            ...
            user.initialize()
            task = Task()
            timeweight = rand.uniform(0, 1)
            task.setTimeWeight(timeweight)
    ...
    for i in range(0,len(user_ranges)):
        ...
        (average_localexecution_energys[i] - average_cen_optimalexecution_energys[i]) / average_localexecution_energys[
            i] * 100))
        ...
        (average_directTocloud_overheads[i] - average_cen_optimalexecution_overheads[i]) / average_directTocloud_overheads[
            i] * 100))
        result_file.write(
