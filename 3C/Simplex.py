...
def Energy_min(users,tasks):
    ...
    for k in range(task_num):
        prob_energy_min +=lpSum(variables['down'+str(k)+','+str(i)]*users[i].DownloadingCost(tasks[k].output_block_num*tasks[k].content.block_size) for i in tasks[k].avalible_users)

        prob_energy_min +=lpSum([variables['input'+str(k)+','+str(i)+','+str(j)]*users[i].InputCost(users[j],tasks[k].output_block_num*tasks[k].content.block_size)[0] for j in users[i].avalibleCooperators] for i in tasks[k].avalible_users)

        prob_energy_min +=lpSum(variables['cpu'+str(k)+','+str(i)] * users[i].ComputingCost(tasks[k].output_block_num*tasks[k].content.block_size,tasks[k]) for i in tasks[k].avalible_users)

        prob_energy_min += lpSum([variables['output'+str(k)+','+str(i)+','+str(j)]* users[i].OutputCost(users[j],tasks[k].output_block_num*tasks[k].content.block_size)[0] for j in users[i].avalibleCooperators] for i in tasks[k].avalible_users)
    ...
    for i in range(user_num):
        ...
        prob_energy_min += lpSum([
            variables['output' + str(k) + ',' + str(j) + ',' + str(i)] * tasks[k].output_block_num * tasks[
                k].content.block_size for j in tasks[k].avalible_users] for k in users[i].avalibleTasks) <= users[i].upload_cellular_data_rate
