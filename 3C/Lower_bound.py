...
def create_LBgraph(tasks,users):
    ...
    MC_graph.add_node('source',demand=0)
    ...
    MC_graph.add_node('destination', demand=total_task_content)

    for task in tasks:
        task_node='task'+str(task.task_id)
        MC_graph.add_node(task_node, demand=-1*task.output_block_num*task.content.block_size)
        MC_graph.add_edge('source', task_node, capacity=task.output_block_num*task.content.block_size, weight=0, caching_user_cost=0,
        ...
        for caching_user_id in task.caching_users:
            ...
            if MC_graph.has_node(caching_node):
                continue
            MC_graph.add_node(caching_node,demand=0)
            MC_graph.add_edge(task_node,caching_node,capacity=task.output_block_num*task.content.block_size,weight=0,caching_user_cost=0,computing_user_cost=0,relaying_user_cost=0)

        MC_graph.add_edge(task_node, 'IOTplatform', capacity=task.output_block_num*task.content.block_size, weight=0,caching_user_cost=0,computing_user_cost=0,relaying_user_cost=0)
        ...
        for computing_user_id in task.avalible_users:
            ...
            if MC_graph.has_node(computing_node):
                continue
            ...
            MC_graph.add_edge('IOTplatform', computing_node, capacity=downloading_capacity, weight=bit_cost,
            ...
            for caching_user_id in task.caching_users:
                if caching_user_id in users[computing_user_id].avalibleCooperators:
                    caching_node='caching'+str(caching_user_id)
                    if MC_graph.has_edge(caching_node, computing_node):
                        continue
                    ...
                    MC_graph.add_edge(caching_node, computing_node, capacity=input_capacity, weight=bit_cost,
        ...
        for relaying_user_id in task.avalible_users:
            ...
            if MC_graph.has_node(relaying_node):
                continue
            ...
            for computing_user_id in task.avalible_users:
                if computing_user_id in users[relaying_user_id].avalibleCooperators:
                    computing_node='computing'+str(computing_user_id)+"'"
                    if MC_graph.has_edge(computing_node,relaying_node):
                        continue
                    ...
                    MC_graph.add_edge(computing_node,relaying_node,capacity=output_capacity, weight=bit_cost,caching_user_cost=0,computing_user_cost=computing_user_cost,relaying_user_cost=relaying_user_cost)
